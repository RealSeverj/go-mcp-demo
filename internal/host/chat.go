...
func (h *Host) StreamChat(
	...
	// 加用户消息
	hist = append(hist, ollama.Message{Role: "user", Content: userMsg})

	tools := h.mcpCli.ConvertToolsToOllama()
	opts := ollama.BuildOptions()
	// 首次流式：边生成边推，遇到 tool_calls 停止
	var assistantBuf string
	var toolCalls []ollama.ToolCall

	err := h.ollamaCli.ChatStream(ctx, ollama.ChatRequest{
		Model:     config.Ollama.Model,
		Messages:  hist,
		Tools:     tools,
		Options:   opts,
	...
	// 把模型已生成的片段先落历史
	if assistantBuf != "" {
		hist = append(hist, ollama.Message{Role: "assistant", Content: assistantBuf})
	}
	...
	for _, tc := range toolCalls {
		args, err := ollama.ParseToolArguments(tc.Function.Arguments)
		if err != nil {
			args = map[string]any{"_error": err.Error()}
		}
		...
		// 工具结果落历史
		hist = append(hist, ollama.Message{
			Role:     "tool",
	...
	// 7) 结束收尾：保存历史、发 done
	if finalBuf != "" {
		hist = append(hist, ollama.Message{Role: "assistant", Content: finalBuf})
	}
	history[id] = hist
